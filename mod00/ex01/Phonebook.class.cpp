...
Phonebook::Phonebook(void) : _indexed(0)
{
}
...
** variable array called _input_data and another static const array called
** _input_msg, each with five string instances. The data strings are for storing
...
** string instances in the _input_data and _input_msg arrays. These arrays
...
** _input_data instance (as defined by x) within the current contact (as defined
...
void	Phonebook::addContact(void)
{
	...
	while (field < TOTAL_FIELD_NUM)
	{
		...
		if (cin_buff_overflow(FIELD_BUFF_SIZE))
			...
			this->_contact[index].setFieldData(field++, buff);
	}
	index++;
	if (this->_indexed < CONTACT_MAX)
...
**
** First we check the _indexed variable to determine whether any contacts have
...
bool	Phonebook::_print_contact_list(void)
{
	if (this->_indexed < 1)
	{
	...
	for (size_t index = 0; index < this->_indexed; index++)
	{
		std::cout << "|" << std::setw(10) << index+1 << "|";
		for (int field = Phonebook::First_Name; field <= Phonebook::Nickname; field++)
		{
			if (this->_contact[index].getFieldDataLength(field) <= 10)
			{
				std::cout << std::setw(10);
				this->_contact[index].showFieldData(field);
			}
			else
			{
				this->_contact[index].showFieldData(field, 9);
				std::cout << ".";
			}
			std::cout << "|";
		}
...
** initialization and saved as the _index_buf_size constant. If the user inputs
...
int		Phonebook::_prompt_for_contact_index(void)
{
	char	buff[Phonebook::_index_buf_size];
	int	index;
	...
	while (1)
	{
		...
		if (cin_buff_overflow(Phonebook::_index_buf_size))
			std::cout << RED << "Too many characters. Use " << Phonebook::_index_buf_size-1 \
			<< " characters or less." << RESET << std::endl;
		else if (isNumber(buff) == false)
			std::cout << RED << "Not a number" << RESET << std::endl;
		else if ((index = std::atoi(buff)) < 1 || (size_t)index > this->_indexed)
...
void	Phonebook::_print_contact_details(int index)
{
	for (int field = Phonebook::First_Name; field < TOTAL_FIELD_NUM; field++)
	{
			std::cout << GRN;
			Contact::showFieldPrompt(field);
			this->_contact[index-1].showFieldData(field);
...
void	Phonebook::searchContact(void)
{
	...
	if (this->_print_contact_list())
		this->_print_contact_details(this->_prompt_for_contact_index());
	return ;
}

void	Phonebook::changeIndexed(int n)
{
	if (this->_indexed + n <= CONTACT_MAX)
		this->_indexed += n;
	return ;
}
...
const size_t Phonebook::_index_buf_size = calculate_index_buf_size(CONTACT_MAX);
